  ...
  return (
    <div className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-elevated)] p-4">
      <div className="flex items-start justify-between">
        <h3 className="text-sm font-semibold text-[var(--foreground)]">Capacidad</h3>
        <Badge label={label} tone={tone} />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-4">
        <div>
          <p className="text-xs text-[var(--color-neutral-500)]">Aforo máximo</p>
          <p className="mt-0.5 text-2xl font-semibold tabular-nums text-[var(--foreground)]">
            {hasGuests ? maxGuests : "—"}
          </p>
        </div>
        <div>
          <p className="text-xs text-[var(--color-neutral-500)]">Capacidad en camas</p>
          <p className="mt-0.5 text-2xl font-semibold tabular-nums text-[var(--foreground)]">
      ...
      {mismatch && (
        <p className="mt-3 text-xs text-[var(--color-warning-700)]">
