...
- **Liora Design Replatform EN CURSO (Fase 16)**: 8 ramas 16A–16H + sub-ramas. Merged: 16A–16D, 16D.5 governance, 16E.5 (paridad de todos los módulos de contenido), 16E.6 (access cockpit), 16F.5 (operator shell, #116), 16F.6 (editor autosave, #118). **Siguiente: Fase 16I — pulido de contenido** (10 ramas, una por pestaña, secuenciales, con aprobación por PR). Pendientes posteriores: 16F assistant `/ai`, 16G legacy removal, 16H guest rewrite. Ver sección "Replatform de diseño (Liora)" abajo + `docs/MASTER_PLAN_V2.md § FASE 16` (+ `§ FASE 16I`) + `docs/ROADMAP.md` + `docs/ARCHITECTURE_OVERVIEW.md` §14.

## Replatform de diseño (Liora) — reglas duras

Replatform visual integral **en ejecución avanzada** (`docs/MASTER_PLAN_V2.md` § FASE 16) — 8 ramas 16A–16H + sub-ramas; 16A–16F.6 merged (ver intro arriba), siguiente **Fase 16I** pulido de contenido. El paquete de diseño vive en `design-system/` (kebab-case, trackeado en Git): `design-system/foundations/` (tokens en 3 capas + `base.css` + `themes.css` + `tailwind.tokens.ts` + `shadcn.css`) es la única fuente de paleta; `design-system/references/liora-ui-kits/` contiene los HTML kits operator/messaging/guest como referencia visual de layout (no de paleta — el azul-gris frío de los kits no se adopta como brand). Validado en CI por `npm run validate:design-system`. Decisiones permanentes (path canónico, dark mode global vía `html[data-theme]`, brand themes guest permanentes, shadcn no wholesale, command palette **ya implementado en 16F.5**, etc.) en `docs/MASTER_PLAN_V2.md` § FASE 16. Reglas duras vigentes en toda rama Liora (incl. 16I) y en cualquier rama funcional intermedia:
...
- Los docs `docs/LIORA_*.md` (`LIORA_DESIGN_ADOPTION_PLAN.md`, `LIORA_COMPONENT_MAPPING.md`, `LIORA_SURFACE_ROLLOUT_PLAN.md`) **ya existen** (creados en 16A/B/C) y se actualizan al cerrar cada rama. No existen `LIORA_MIGRATION_RULES.md` ni `*_TEMPLATE.md` (nombres tentativos que nunca se materializaron — reglas de migración en `design-system/docs/DESIGN_MIGRATION.md`).
...
## Patrones de UI — Operator shell (Fase 16D / 16D.5 / 16F.5 / 16F.6)

- **Auto-save de editores** (`src/lib/use-form-auto-save.ts`, Fase 16F.5 → completado en 16F.6): `useFormAutoSave(formRef, delay=700, watch?: () => string): flush` — los editores de sección guardan al editar (como Acceso), **sin botón "Guardar" ni texto "Guardado"**; las tarjetas con modo edición cierran con **"Listo"** (o el chevron de colapso). Mecánica: efecto por-render que diffea `serializeForm(FormData)` + listeners nativos `input`/`change` → `requestSubmit()` debounced, gateado por `checkValidity()` (sin popups; nunca guarda en mount — baseline). `<AutoSaveStatus pending={...} />` muestra "Guardando…" o nada. Tres extensiones de 16F.6: (1) **`watch`** = señal de cambio para forms cuyo payload se construye estado→JSON sin inputs con `name` (Normas `buildPoliciesJson`, Sistemas/Equipamiento) — cuando se pasa es la señal **autoritativa** (se omite `serializeForm`); (2) **re-attach/re-baseline** al cambiar `formRef.current` → soporta forms montados condicionalmente (edit-cards, cuerpos colapsables) sin extraer sub-componentes; (3) **`flush()` de retorno + flush-on-unmount** → cero pérdida al cerrar/navegar (el handler "Listo"/colapso llama `flush()` síncrono; el cleanup hace flush de saves pendientes; idempotente vía `pendingRef`). **`required` HTML es incompatible con guardado incremental** (`requestSubmit()` rehúsa forms inválidos): los renderers de campo que auto-guardan parcial pasan `{...field, required:false}` (Sistemas `FieldInput`, Equipamiento `SubtypeFieldInput`) conservando el asterisco `*` como hint; la action persiste JSON parcial. Patrón hidden-mirror para steppers fuera del form: `<form className="contents">` con solo inputs hidden (Camas cantidad). Consumidores: property/settings/troubleshooting/incidents (16F.5) + policies/systems/local-guide/contacts/knowledge/amenities/spaces/bed-manager (16F.6).
...
- **AppShell** (`src/components/layout/app-shell.tsx`): server component. Monta `SideNav` + columna de contenido (`Topbar` + `<main>` con el contenedor fluido `--content-max`) + los tiradores `NavDrawerTab`/`RailDrawerTab` (`fixed`). `getDerived()` (sectionScores) + `getOperatorNotifications()` concurrentes vía `Promise.all`, cada uno con `.catch` fail-soft. El `<main>` envuelve en `mx-auto w-full max-w-[var(--content-max)] px-4 pb-16 sm:px-6 lg:px-8` — ancho/gutters del shell, no de cada página.
- **Topbar** (`src/components/layout/topbar.tsx`): client. breadcrumbs | `CommandPalette` (⌘K) | `AssistantLauncher` (⌘J) + `NotificationsPopover` + `Vista huésped` condicional + `ThemeToggle`. **No** hay botón "Publicar" (gateado; vive en el hub).
- **CommandPalette** (`src/components/layout/command-palette.tsx`): ⌘K sobre **Radix Dialog + `fuse.js`** (sin dep nueva). Busca **todo**, no solo secciones: `getOperatorSearchAction(propertyId)` (lazy en el primer open) → índice de `operator-search.service.ts` (secciones + entidades configuradas: contactos, espacios, sistemas, equipamiento, guía local, soluciones + conceptos de normas); cada resultado deep-linka a su sección. Query vacía → secciones. Anclas por-fila (saltar a una fila concreta) = follow-up `FUTURE.md §8.2`.
- **AssistantLauncher** (`src/components/layout/assistant-launcher.tsx`): el Asistente IA es herramienta-compañera, **fuera del nav** — botón en el topbar + drawer derecho (**Radix Dialog**: focus trap + scroll lock + Escape; atajo ⌘J) que monta `AssistantChat` + link a Conocimiento. `ai` y `knowledge` son `hideFromNav` (rutas vivas; el conocimiento es mayormente auto-extraído).
- **NotificationsPopover** (`src/components/layout/notifications-popover.tsx`): feed read-only (bloqueos de publicación + incidencias abiertas) de `operator-notifications.service.ts` (usa `getValidationsForProperty` cache()'d), pasado AppShell→Topbar. Bell = `<IconButton>` + badge; dropdown vía `useDismiss` (`src/lib/use-dismiss.ts`, hook compartido click-outside+Escape; las superficies **modales** usan Radix Dialog). Drafts/stale = follow-up.
- **Colapso + resize de paneles** (`src/components/layout/shell-chrome.tsx`): ambos paneles, mismo sistema. El **ancho** vive en CSS vars inline en `<html>` (`--sidebar-width`/`--rail-width`), seteadas por el pre-paint de `layout.tsx` + los toggles + el resizer; los **atributos** `html[data-nav-collapsed]`/`[data-rail-collapsed]` (`shell.css`) gobiernan solo VISIBILIDAD (nunca ancho). Colapso vía `DrawerTab` (tirador centrado verticalmente en el borde interior del panel; nav→icon-rail 56px, rail→oculto 0; renderizado en AppShell, sobrevive al colapso). Resize vía `PanelResizeHandle` (`role="separator"` + `aria-valuenow`, drag + flechas/Home/End + doble-clic reset, clamp min/max, persistido; escribe solo la CSS var en drag, sincroniza estado al soltar). Keys + bounds + copia literal en el pre-paint: `src/lib/shell-prefs.ts`. Clases marker (`shell-nav-label`, `shell-nav-group-label`, `shell-nav-progress`, `shell-prop-switcher`, `shell-rail`, `shell-nav-resizer`, `shell-rail-resizer`, `shell-guest-view-btn`, `shell-nav-footer-link`) acoplan CSS↔componentes (sin test que lo guarde; follow-up `FUTURE.md §8.5`).
- **SideNav** (`src/components/layout/side-nav.tsx`): `NAV_ICONS` local (NO en `navigation.ts`). **3 grupos visibles**: CONTENIDO · SALIDAS · OPERACIONES — el grupo `assistant` queda sin ítems (ai/knowledge en el drawer) y se filtra. El colapso lo dispara el `NavDrawerTab` (tirador), no un toggle inline; resize por `NavResizeHandle` (solo desktop). Header = property switcher; footer = "Nueva propiedad".
...
### Nav IA + ModuleContainer (Fase 16F.5)

- **Nav config-driven, 3 grupos visibles**: `src/lib/navigation.ts` resuelve `WORKSPACE_NAV` desde `SECTION_EDITORS` + `EXTRA_ITEMS` (no-section: overview, publishing hub, reservations, incidents, analytics, settings). `NAV_ORDER` fija el orden curado; un section editor no listado y no `hideFromNav` se auto-añade a su grupo (contrato config-driven "añadir sección = nav item"). El tipo `NavGroup` mantiene 4 valores (`content | assistant | publishing | operations`) pero **`assistant` no tiene ítems visibles** (ai/knowledge en el drawer) y `SideNav` filtra grupos vacíos → se renderizan **CONTENIDO · SALIDAS · OPERACIONES** (labels `NAV_GROUP_LABELS`; `publishing` = "Salidas").
  - **Orden Contenido**: Resumen · Propiedad · Acceso · **Espacios · Sistemas · Equipamiento** · Guía local · Soluciones · Normas · Contactos. Espacios va **antes** que Sistemas/Equipamiento por dependencia dura del modelo: la cobertura de sistemas es por-espacio (`PropertySystemCoverage.spaceId`) y el equipamiento es por-espacio.
  - **Salidas**: "Guía del huésped" (`publishing` hub: preview + publicar + versiones + QR). **Operaciones**: Reservas · Mensajería · Incidencias · Operativa · Media · Analítica · Configuración.
  - **Renombrados/movidos (16F.5)**: `troubleshooting` label **"Soluciones"** (playbooks guest-facing que alimentan guía + IA; las ocurrencias reales viven en Operaciones · Incidencias, enlazadas desde `/incidents`); `publishing` item **"Guía del huésped"**; `messaging` → grupo `operations`. `section-editors.ts` lleva `hideFromNav?`: `guest-guide` (preview, vía "Vista huésped"), `activity` (plegada en Configuración), `ai` + `knowledge` (en el drawer del Asistente) — todos con ruta viva.
- **Búsqueda integral** (`src/lib/services/operator-search.service.ts` + `src/lib/actions/operator-search.actions.ts`): el ⌘K indexa por-propiedad secciones + entidades configuradas (contactos, espacios, sistemas, equipamiento, guía local, soluciones) + conceptos de normas (taxonomía → discoverability de "fumar", "mascotas"…), cada uno con deep link a su sección. Read-only, gateado por `requireOperator()` + ownership scope. La asimetría (normas = catálogo de taxonomía vs el resto = configurado) es decisión de discoverability documentada.
- **`<ModuleContainer>`** (`src/components/layout/module-container.tsx`): contenedor único de página operator — renderiza el `<PageHeader>` sticky compartido + el body. El ancho/gutters/padding-inferior los posee el `<main>` del AppShell (no el contenedor). Páginas con header inline lo usan; las que ya tienen `<PageHeader>` dentro de un form (access/contacts/policies/property) heredan el mismo comportamiento sin envolver.
- **`<PageHeader sticky>`** (`src/components/ui/page-header.tsx`): `sticky` **default true** — pega el header bajo el topbar (`top: var(--topbar-height)`, z-20) con fondo sólido + border-b alineados a la columna (sin bleed de negative-margin, robusto cuando va anidado en un form). Único consumidor: surfaces operator. El body de `ModuleContainer` neutraliza el `mt-*` del primer hijo (`[&>*:first-child]:mt-0`) para no duplicar el gap.
- **`getValidationsForProperty(propertyId)`** (`src/lib/validations/run-all.ts`): wrapper React `cache()` de `runAllValidations(propertyId)` para deduplicar la pasada de validaciones cuando co-renderizan en el mismo request (overview + feed de notificaciones; publishing + feed). Callers con la Property ya cargada siguen usando `runAllValidations(id, preloaded)`.
- **Páginas legacy con migración parcial (16F.5)**: páginas ad-hoc no auditadas (knowledge, ai, ops, media, analytics, reservations, incidents, local-guide, settings) recibieron `<ModuleContainer>` en el header (consistencia de shell + tokens semánticos en el header) pero conservan tokens legacy en el body — **no** se añadieron a `AUDITED_SURFACES`; paridad visual completa por módulo diferida (patrón "deferred visual parity"). `troubleshooting`/overview SÍ están auditadas y pasan invariantes.
