...
- **AppShell** (`src/components/layout/app-shell.tsx`): server component. Monta `SideNav` + columna de contenido (`Topbar` + `<main>` con el contenedor fluido `--content-max`). `getDerived()` (sectionScores) + `getOperatorNotifications()` se resuelven **concurrentes** vía `Promise.all`, cada uno con `.catch` fail-soft (un fallo no rompe la navegación). El `<main>` envuelve en `mx-auto w-full max-w-[var(--content-max)] px-4 pb-16 sm:px-6 lg:px-8` — el ancho/gutters son del shell, no de cada página (16F.5).
- **Topbar** (`src/components/layout/topbar.tsx`): client component. breadcrumbs | `CommandPalette` (⌘K real) | `NotificationsPopover` + `Vista huésped` condicional + `ThemeToggle`. **No** hay botón "Publicar" (publicación está gateada — vive en el hub). Usa `isNavItemActive()` de `navigation.ts`.
- **CommandPalette** (`src/components/layout/command-palette.tsx`, 16F.5): ⌘K real sobre **Radix Dialog + `fuse.js`** (sin dependencia nueva — `cmdk` descartado). `⌘K`/`Ctrl+K` global (keydown con cleanup) + trigger en el topbar; búsqueda fuzzy sobre `WORKSPACE_NAV` (navegación a secciones); flechas/Enter. Reemplazó el `CommandBarSlot` no-interactivo de 16D (borrado). Scope v2 (contenido + acciones) en `FUTURE.md §8.2`.
- **NotificationsPopover** (`src/components/layout/notifications-popover.tsx`, 16F.5): feed read-only de bloqueos de publicación + incidencias abiertas, derivado server-side en `operator-notifications.service.ts` (usa `getValidationsForProperty` cache()'d) y pasado por prop AppShell → Topbar. Bell = `<IconButton>` + badge de conteo; dropdown click-outside + Escape (listener con cleanup). Drafts/stale = follow-up (`FUTURE.md §8.4`).
- **Colapso nav + rail** (16F.5): atributo-driven — `html[data-nav-collapsed]` / `html[data-rail-collapsed]` (en `src/styles/shell.css`) gobiernan el layout (sidebar→icon-rail vía `--sidebar-width`; rail oculto + grid track caído). Pre-paint en `layout.tsx` los setea antes del primer paint (sin FOUC; lleva copias literales de los keys `shell:nav-collapsed`/`shell:rail-collapsed` de `src/lib/shell-prefs.ts`). Toggles en `shell-chrome.tsx` (`NavCollapseToggle`/`RailCollapseToggle`, sobre `<IconButton>`) flip-ean el atributo + persisten. **Clases marker** (`shell-nav-label`, `shell-nav-group-label`, `shell-nav-progress`, `shell-prop-switcher`, `shell-rail`, `shell-content-grid`, `shell-guest-view-btn`, `shell-nav-footer*`) acoplan CSS↔componentes — renombrarlas rompe el colapso (sin test que lo guarde aún; follow-up `FUTURE.md §8.5`).
- **SideNav** (`src/components/layout/side-nav.tsx`): iconos en mapa local `NAV_ICONS: Partial<Record<string, LucideIcon>>` — NO en `navigation.ts`. **4 grupos cadena-de-valor**: `content | assistant | publishing | operations` (16F.5). Badge de propiedad arriba (`shell-prop-switcher`), grupos en el centro, footer con "Nueva propiedad" + `NavCollapseToggle`.
...
- **Nav config-driven en 4 grupos**: `src/lib/navigation.ts` resuelve `WORKSPACE_NAV` desde `SECTION_EDITORS` + `EXTRA_ITEMS` (no-section: overview, publishing hub, reservations, incidents, analytics, settings). El orden lo fija `NAV_ORDER` (curado, cadena-de-valor); un section editor no listado y no `hideFromNav` se auto-añade al final de su grupo (preserva el contrato config-driven "añadir sección = nav item"). Grupos: `content | assistant | publishing | operations` (tipo `NavGroup`). `section-editors.ts` lleva `hideFromNav?` — `guest-guide` (preview, vía "Vista huésped") y `activity` (plegada en Configuración) ocultos del nav pero con ruta viva. `troubleshooting` = label **"Averías"** (la ruta sigue `/troubleshooting`); el ítem Operaciones `incidents` mantiene "Incidencias".
