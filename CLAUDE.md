...
- Assistant pipeline (rama 11C): la Postgres local requiere la extensión `vector` (pgvector). La migración `20260419180000_add_knowledge_embeddings_and_fts` ejecuta `CREATE EXTENSION IF NOT EXISTS vector` (idempotente) pero el rol debe tener permiso para crearla la primera vez. Verificar: `psql "postgresql://javierabrilibanez@localhost:5432/guide_builder" -c "SELECT extname, extversion FROM pg_extension WHERE extname='vector';"`. La columna `embedding` se declara como `Unsupported("vector(512)")` en `schema.prisma` — el cliente Prisma no la expone; lecturas/escrituras vectoriales van por `$queryRaw`/`$executeRaw` en `src/lib/services/assistant/*`. La columna generada `bm25_tsv` y su índice GIN existen solo en SQL (no en `schema.prisma`); no se consulta vía Prisma client
