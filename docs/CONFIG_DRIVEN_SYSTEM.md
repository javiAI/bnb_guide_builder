...
├── messaging_triggers.json    # Rama 12B — triggers de automation (anchor + offset presets)
├── messaging_starter_packs.json # Rama 12C — 6 packs (tone × locale) con templates + automations inactivas
├── guide_outputs.json
...
| Presenter registry | ✅ | | Guest humanization (10F) |
| Quick-action registry | ✅ | | Hero actions (10G) |
...
- `troubleshooting_taxonomy.json`
- `messaging_touchpoints.json`
- `messaging_triggers.json` — triggers canónicos para `MessageAutomation.triggerType` (rama 12B). Cada entry declara `id` (`before_arrival | day_of_checkin | after_checkout | on_booking_confirmed`), `label`/`description` (ES), `anchorField` ∈ `{checkIn, checkOut, bookingConfirmed}` (**source of truth del anchor — la DB no guarda anchor**), `defaultOffsetMinutes` y `presets[]` (`{label, offsetMinutes}` para chips de UI). Parseada con Zod en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) vía `loadMessagingTriggers()` + helper `findMessagingTrigger(id)`. Consumida por `computeScheduledSendAt()` en [src/lib/services/messaging-automation.service.ts](../src/lib/services/messaging-automation.service.ts) (que resuelve anchor → `fromZonedTime(local, property.timezone)` vía `date-fns-tz`) y por la UI de automations (`TriggerAndOffsetFields` en `automation-section.tsx`). Legacy mapping en [src/lib/schemas/messaging.schema.ts](../src/lib/schemas/messaging.schema.ts) (`normaliseTriggerType`: `reservation_relative → before_arrival`, `on_event → on_booking_confirmed`) — sin migración DB. Añadir un trigger nuevo = editar JSON + `anchorField` tiene que existir en `ANCHOR_FIELDS` del service.
...
- `messaging_starter_packs.json` — packs de arranque (rama 12C). 6 packs = 3 tones (`friendly|formal|luxury`) × 2 locales (`es|en`), mono-locale (`language === locale`), cada uno con 7 templates (uno por touchpoint canónico) + `automation` pre-cableada `active: false`. Shape: `id` (`msp.<tone>_<locale>`), `templates[]` con `touchpointKey`, `channelKey`, `bodyTemplate`, `automation: { triggerType, sendOffsetMinutes }` y `overrides?[]` por `appliesToPropertyTypes` (patch sobre `subjectLine|bodyTemplate`). Parseada con Zod en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) mediante `loadMessagingStarterPacks()` — fail-loud en boot si: `(tone, locale)` duplicado, `language !== locale`, touchpoint/trigger/propertyType desconocidos, token desconocido en body/override, `internal_only` en cualquier body, o `sensitive_prearrival` con `touchpointKey !== "mtp.day_of_checkin"` / `triggerType !== "day_of_checkin"` / `offset ∉ [-2880, 0)` (espejo del gate 12B de 48 h). Consumida por `messaging-seed.service.ts` (`listAvailablePacks`, `previewPack`, `applyStarterPack`) + `StarterPackPicker` UI. Idempotencia por **merge-by-slot** con clave `(propertyId, touchpointKey, channelKey, language)`. Columnas clave: `MessageTemplate.origin` (`user|pack`, default `user`) + `packId` (nullable) + `@@index([propertyId, origin, packId])`. En `$transaction`, `applyStarterPack` carga filas user+pack indexadas por slot; por template: user en slot → skip (nunca compite), pack idéntico (mismo packId + subject + body + automation) → skip (cero writes), pack con drift → update in-place (conserva `id` + historial), slot vacío → create. Al final: `deleteMany` sobre pack rows cuyo slot no produce el nuevo pack. Re-aplicar el mismo pack = no-op real. Edit del host (`updateMessageTemplateAction`) flipa a `origin="user", packId=null` **solo si `bodyMd`/`subjectLine` cambian** (toggles de status/channel no detach) y ese row queda fuera de futuros swaps. Añadir un pack nuevo = editar JSON respetando invariantes; `src/test/messaging-starter-packs.test.ts` cubre apply/re-apply/overrides y el validator.
- `guide_outputs.json`
- `visibility_levels.json`
- `media_requirements.json`
- `media_asset_roles.json`
...
- `review_reasons.json`
- `contact_types.json`
- `escalation_rules.json` — intents del assistant (int.lockout, int.emergency_medical, int.emergency_fire, int.emergency_security, int.general, int.maintenance_plumbing, int.maintenance_electrical) con `matchKeywords.{es,en}[]`, `contactRoles[]` (resolución contra `Contact.roleKey`), `emergencyPriority: boolean`, `channelPriority[]` y `label` (string). Parseada con Zod en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) mediante `loadEscalationRules()`. Consumida por `resolveEscalationIntent()` (heurística) y `resolveEscalation()` (cascada de contactos). Añadir un intent nuevo exige ≥5 rows etiquetados en [escalation-intent-corpus.ts](../src/test/fixtures/escalation-intent-corpus.ts) para pasar el precision gate.
...
- `local_place_categories.json` — catálogo canónico de categorías `lp.*` para `LocalPlace.categoryKey` (rama 13A). 14 entries (restaurant, cafe, bar, supermarket, pharmacy, hospital, transport, parking, attraction, beach, park, gym, laundry, other). Shape: `id` (`lp.<slug>`), `label` (ES), `description`, `guestLabel`, `icon`, `recommended` (boolean). Parseada con Zod en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) mediante `loadLocalPlaceCategories()` — fail-loud en boot si hay `id` duplicados o algún `id` no cumple `lp.<slug>`. Helpers: `findLocalPlaceCategory(id)`, `isLocalPlaceCategoryKey(id)` (usado por el Zod refine de `createLocalPlaceSchema` — rechaza rows sin prefijo `lp.*`). La migración 13A backfillea legacy unprefixed (`restaurant` → `lp.restaurant`, etc.) vía un `UPDATE ... SET ... CASE` y parquea cualquier no-`lp.*` residual bajo `lp.other`. El prefijo `lp.` ya está en `FALLBACK_ALLOWED_PREFIXES` del presenter (resuelve a `genericTextPresenter`). Provider-mapped POIs llegan con `categoryKey` ya en forma canónica — ver `src/lib/services/places/maptiler-category-mapping.ts`.
- `amenity_taxonomy.json`, `property_types.json`, `space_types.json`, `access_methods.json`, `policy_taxonomy.json` — **mappings de plataforma obligatorios** (rama 14A, reformulada). El vocabulario OTA de Airbnb y Booking es heterogéneo — algunos conceptos son catálogos planos, otros son campos estructurados del listing, otros van a buckets free-text, otros contribuyen a contadores de habitaciones. Forzar todo a un único shape `{platform, external_id}` obligaba a marcar como `platform_supported:false` decenas de ítems que sí tienen representación real en plataforma. El shape canónico es una **union discriminada por `kind`**:
  - `{platform, kind: "external_id", external_id}` — catálogo plano (amenity IDs, PCT codes).
  - `{platform, kind: "structured_field", field, transform}` — campo estructurado del listing (`pricing.cleaning_fee`, `listing_policies.pets_allowed`, `person_capacity`); `transform` es `"bool" | "currency" | "minutes" | "enum" | "number"` y es **obligatorio** (14B lo necesita para saber cómo serializar).
  - `{platform, kind: "free_text", field}` — bucket de texto libre del listing (`house_rules`, `checkin_instructions`) cuando no hay catálogo.
  - `{platform, kind: "room_counter", counter}` — contribuye a un contador de habitaciones (`bedrooms`, `bathrooms`, `beds`), no es un ítem de catálogo.

  Kinds permitidos por taxonomía (enforced por el gate test): `amenities` (`external_id | structured_field`), `property_types` (`external_id`), `access_methods` (`external_id | free_text`), `space_types` (`room_counter | structured_field`), `policies` (`structured_field | free_text`). VRBO se admite en el union de `PlatformId` pero **no** satisface el invariante — la cobertura exige Airbnb **o** Booking.

  Estados admitidos por ítem clasificable: (a) ≥1 entry bien formada en `source[]` cuyo `platform` sea `"airbnb"` o `"booking"`, **o** (b) `platform_supported: false` cuando el concepto no existe en ninguna de las dos plataformas. La ambigüedad (ni mapping ni flag) falla el build. Exclusiones: grupos `ag.*` de `amenity_taxonomy` y los parent `pol.house_rules`/`pol.fees` de `policy_taxonomy` son navegacionales, no clasificables. `source_refs` del archivo es una tabla separada de URLs de documentación (no mappings).

  Helpers en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts): `getAirbnbMapping(taxonomy, id)` / `getBookingMapping(taxonomy, id)` devuelven el `PlatformMapping` completo (o `null`); 14B/14C hacen switch sobre `kind`. Sugar legacy: `getAirbnbId(taxonomy, id)` / `getBookingId(taxonomy, id)` devuelven el `external_id` sólo cuando `kind === "external_id"` — `null` para cualquier otro kind (un `structured_field` no es un ID de catálogo). Los cuatro helpers throw para IDs desconocidos. `validatePlatformMapping(entry)` es la validación de shape compartida entre runtime y el gate test (single source of truth). Gate forward: `src/test/platform-mappings-coverage.test.ts` (28 tests).

  **Reverse coverage gate** (14A): el gate forward valida que cada ítem nuestro esté mapeado o `platform_supported:false`, pero no detecta lagunas en la otra dirección — si Airbnb expone un amenity ID que ninguna de nuestras taxonomías referencia, el forward gate nunca lo ve. El directorio [taxonomies/platform-catalogs/](../taxonomies/platform-catalogs/) pinnea el vocabulario oficial de Airbnb y Booking (`airbnb-amenities.json`, `booking-amenities.json`, `airbnb-property-types.json`, `booking-property-types.json`, `airbnb-access-methods.json`) más dos manifests de campos estructurados (`airbnb-structured-fields.json`, `booking-structured-fields.json`). Cada entry es `relevance: "covered"` o `relevance: "out_of_scope"` con `reason` normalizada (`hotel_only | deprecated | duplicate | not_relevant_to_str | covered_via_alias | platform_not_actionable`); `deferred` no existe al final de la rama (rechazado por el test). `src/test/platform-reverse-coverage.test.ts` (42 tests) valida: (1) toda entry `covered` del catálogo/manifest está referenciada por ≥1 ítem taxonómico, (2) shape pinneado (`pinned_at`, `source_urls[]`, `scope`, `entries[]`), (3) `collapsed_to` (alias muchos-a-uno, e.g. `cottage/chalet/cabin → pt.house`) apunta a un ítem existente que carga el `external_id` declarado, (4) `out_of_scope` obligatorio con `reason` del set normalizado. Ver [taxonomies/platform-catalogs/README.md](../taxonomies/platform-catalogs/README.md) para política de actualización de catálogos y semántica de `collapsed_to`.
- `incident_categories.json` — catálogo canónico de categorías para `Incident.categoryKey` (rama 13D). 9 entries (wifi, access, appliance, cleaning, safety, noise, supplies, plumbing, other) usadas por el guest reporter (chip selector en `IssueReporter`) y el host panel (filtro + badge en list/detail). Shape: `id` (`ic.<slug>`), `label` (ES, host-facing), `guestLabel` (ES, neutro para guest), `description`, `icon`, `hostDefaultSeverity` (`low|medium|high`). Parseada con Zod en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) mediante `loadIncidentCategories()` — fail-loud en boot si hay `id` duplicados o algún `id` no cumple `ic.<slug>`. Helpers: `findIncidentCategory(id)`, `isIncidentCategoryKey(id)` (usado por el Zod refine de `createGuestIncidentSchema`). `Incident.categoryKey` es **FK suave** (string nullable, no FK hard al no ser una tabla DB) porque la taxonomía vive en JSON; el invariante de validación se aplica en el server action antes de `prisma.incident.create`. El guest ve solo `categoryKey` en la tracking page proyectado a `guestLabel` — el field whitelist de `visibility.ts` garantiza que ningún campo sensible del incident llega a `/g/*`.
...
| Añadir una quick action al hero (rama 10G) | Sí (1 entrada en registry + 1 key en `guide_sections.json`) | Declarar `QuickAction` en `quick-action-registry.ts`; listar su id en `quickActionKeys` de la sección; `validateSectionReferences()` en boot falla si la clave no existe o se duplica. Si `resolve` retorna `null`, el botón se omite (graceful degradation) |
...
El sistema config-driven está diseñado para sobrevivir a un replatform visual integral (Fase 16 Liora) sin tocar taxonomías, engines ni resolvers. El punto de inyección es triple:
