...
- `review_reasons.json`
- `contact_types.json`
- `escalation_rules.json` — intents del assistant (int.lockout, int.emergency_medical, int.emergency_fire, int.emergency_security, int.general, int.maintenance) con `keywords.{es,en}[]`, `contactRoles[]` (resolución a `Contact.contactType`), `emergency: boolean` y `label.{es,en}`. Parseada con Zod en el loader ([escalation-rules.loader.ts](../src/lib/taxonomy/escalation-rules.loader.ts)). Consumida por `resolveEscalationIntent()` (heurística) y `resolveEscalation()` (cascada de contactos). Añadir un intent nuevo exige ≥5 rows etiquetados en [escalation-intent-corpus.ts](../src/test/fixtures/escalation-intent-corpus.ts) para pasar el precision gate.
