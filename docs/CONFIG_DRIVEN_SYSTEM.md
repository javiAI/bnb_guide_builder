...
- `amenity_taxonomy.json`, `property_types.json`, `space_types.json`, `access_methods.json`, `policy_taxonomy.json` — **mappings de plataforma obligatorios** (rama 14A). Cada ítem clasificable debe declarar exactamente uno de: (a) ≥1 entry en `source[]` con forma canónica `{platform: "airbnb"|"booking"|"vrbo", external_id: string}` (mapping parcial permitido — sólo Airbnb o sólo Booking), o (b) `platform_supported: false` cuando el concepto no existe en el vocabulario OTA. La ambigüedad (ni mapping ni flag) rompe el contrato de export/import y falla el build. Exclusiones: grupos `ag.*` de `amenity_taxonomy` y los parent `pol.house_rules`/`pol.fees` de `policy_taxonomy` son navegacionales, no clasificables. `source_refs` del archivo es una tabla separada de URLs de documentación (no mappings). Helpers: `getAirbnbId(taxonomy, id)` / `getBookingId(taxonomy, id)` en [src/lib/taxonomy-loader.ts](../src/lib/taxonomy-loader.ts) — throw para IDs desconocidos, `null` cuando no hay mapping para esa plataforma. Gate: `src/test/platform-mappings-coverage.test.ts`.
