...
- Plain-button dot row when `1 < slides.length ≤ MAX_VISIBLE_DOTS` (5): each dot is a `<button>` at 32×32 visual + `recipe-icon-btn-32` slop (`::before { inset: -6px }` → 44×44 hit on fine pointers; `@media (pointer: coarse)` collapses to 44×44 visual). Keeps the chip compact (~32 tall) so it doesn't dominate the cover. `aria-current="true"` on active, `aria-label="Mostrar <slide title>"`, ArrowLeft/Right + Home/End nav with wrap. Container marked `data-carousel-indicator="dots"`. **No `role="tab"` / `aria-selected`** — independent media indicators, not a tabs pattern.
...
- Interactive indicators when `slides.length > 1`. **Two modes** switched at `MAX_VISIBLE_DOTS = 5` (see `src/components/ui/media-carousel.tsx`):
  - **Dot row** (1 < N ≤ 5): plain `<button>` controls at 32×32 visual + `recipe-icon-btn-32` slop (44×44 hit on fine pointers; 44×44 visual on coarse pointers). Inner active indicator is a 1px-tall span — chip stays subtle against the cover image. `aria-current="true"` on active, keyboard nav (ArrowLeft/Right + Home/End with wrap). Container marked `data-carousel-indicator="dots"`. **No `role="tab"` / `role="tablist"` / `aria-selected`** — dots are independent media indicators, not a tabs pattern.
  - **Compact strip** (N > 5): `<button aria-label="Slide anterior">` ← `<span aria-live="polite" data-carousel-counter>{N}/{M}</span>` → `<button aria-label="Slide siguiente">`. Arrows reuse the same 32 visual + `recipe-icon-btn-32` slop so both modes ship a chip of identical visual height. Counter announces position to assistive tech. Wrap-around on both arrows (prev from first → last, next from last → first). Keyboard reach to any slide: repeated arrow press, ArrowLeft/Right cycles, Home/End jumps to first/last. Container marked `data-carousel-indicator="compact"`.
  - **Why the switch**: 5 buttons × 32 px + gaps ≈ 170 px, which fits comfortably inside a narrow card column at 1280 px (cockpit 1×4 layout, ~245 px per card). Above that the dot row would clip or wrap. The compact strip's fixed width is independent of slide count, so a card adopting this primitive will never overflow regardless of how many slides accumulate.
  - **Why the slop, not visual 44**: chip dominates the cover at h-11 (44 px is a third of the 140 px collapsed media area). `recipe-icon-btn-32` is the CLAUDE.md-documented accepted pattern for icon-only desktop buttons — 32 visual + ::before inset -6px → 44 hit on fine pointers, coarse-pointer media query collapses to 44 visual on touch. The deprecated `recipe-dot-24` (24 visual, never reached 44 reliably) is forbidden and must not be reintroduced.
  - **Reusable contract** for `spaces/` and any future media-backed entity surface that consumes `<MediaCarousel>`: don't fork the indicator pattern, don't hardcode a different threshold. `MAX_VISIBLE_DOTS` is the single source of truth. New surfaces inherit overflow behavior for free.
...
- **16E.6 closure note (PR #106)**: the rama expanded beyond its planned scope (orchestrator-approved, no penalty). Final patterns pinned for downstream consumers — promote to `CLAUDE.md` when a second surface adopts them:
  ...
  - **`<MediaCarousel>` indicator contract**: `MAX_VISIBLE_DOTS = 5` is the sole switching threshold (constant in `src/components/ui/media-carousel.tsx`). At ≤5 slides the carousel renders the per-slide dot row (`data-carousel-indicator="dots"`, real 44×44 buttons). At >5 slides it switches to a compact strip (`data-carousel-indicator="compact"`) with two arrows + `aria-live="polite"` counter (`{N}/{M}`). Both arrows wrap (prev from first → last, next from last → first); Home/End jump to first/last. Every clickable in either mode is a real 44×44 `<button>` (`h-11 w-11`) — no `recipe-dot-24`, no pseudo-element slop. The recipe was removed in the fourth-pass review and must not be reintroduced. Reusable for `spaces/` and any future media-backed surface; do not fork the threshold.
