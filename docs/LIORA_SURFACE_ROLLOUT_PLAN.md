...
- Plain-button dot row when `1 < slides.length ≤ MAX_VISIBLE_DOTS` (5): each dot is a **real 44×44** `<button>` (`h-11 w-11`, no pseudo-element slop), `aria-current="true"` on active, `aria-label="Mostrar <slide title>"`, ArrowLeft/Right + Home/End nav with wrap. Container marked `data-carousel-indicator="dots"`. **No `role="tab"` / `aria-selected`** — independent media indicators, not a tabs pattern.
