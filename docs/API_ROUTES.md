...
| `/api/properties/:propertyId/import/airbnb/apply` | POST | `mutate` |
| `/api/properties/:propertyId/import/booking/apply` | POST | `mutate` |
...
- `POST /api/properties/:propertyId/import/airbnb/apply` — applies the previewed diff to the Property (rama 15E). Body: `{ payload: unknown, resolutions: Record<string, "take_import" | "keep_current" | "skip"> }`. The server **recomputes its own diff** from `payload` (never trusts a client-shipped diff) and reconciles with `resolutions`. Success → `200 { data: { result: "success", payloadFingerprint, applied[], skipped[], warnings[] } }` or `200 { data: { result: "noop", payloadFingerprint } }` (idempotent replay). Error responses: `400 INVALID_JSON`, `400 INVALID_BODY` (Zod), `400 INVALID_PAYLOAD` (parse), `400 INVALID_RESOLUTION` (resolution targets a non-actionable category — `presence.*`, `freeText.*`, `customs.*`), `409 STALE_RESOLUTIONS` with `{ diff, missingFields[] }` (server diff no longer contains the resolved field), `500 APPLY_FAILED` with `{ payloadFingerprint, message }`. The transaction takes a `SELECT … FOR UPDATE` row lock on the Property to defend `policiesJson` merge under concurrent writes. `noop` rows are NOT audited (only `console.info`); `success` and `failed` both write an `AuditLog` row with `action=import.apply`. **Access control** (15A–15D): wrapped in `withOperatorGuards({ rateLimit: "mutate" })`. See [docs/FEATURES/PLATFORM_INTEGRATIONS.md](FEATURES/PLATFORM_INTEGRATIONS.md) §12.
- `POST /api/properties/:propertyId/import/booking/apply` — symmetric to `/import/airbnb/apply` for Booking listings (rama 15E). Same body shape, error responses, and access control wrapper.
