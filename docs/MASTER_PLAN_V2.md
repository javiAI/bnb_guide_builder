...
- **Discovery cockpit unificado** (planificado: solo parking). Real: `parking` + 6 modos de transit (`airport`, `train`, `bus`, `metro`, `urban_bus`, `taxi`). El cockpit S02 ("Cómo llegar") toggea cada modo y dispara `searchNearbyArrivalOptionsAction(mode)` con la misma cadencia (bucket `expensive` + per-property limiter + cooldown UI). Persistencia: `LocalPlace` con `categoryKey` mapeado del modo (`lp.transport.airport`, `lp.transport.train`, etc.) — mismo modelo que parking. Cache de sugerencias por modo en columna nueva `Property.arrivalSuggestionsCacheJson` (`Partial<Record<ArrivalMode, ArrivalSuggestion[]>>`) + `Property.arrivalModesEnabledJson` (per-mode enable/disable persistente).
...
- **6 modos de transit + parking** en el cockpit "Cómo llegar". El set canónico está en `arrivalModeEnabledSchema` (`parking | airport | train | bus | metro | urban_bus | taxi`). Cualquier nueva surface que itere sobre modos debe consumir ese schema, no hardcodear el array.
