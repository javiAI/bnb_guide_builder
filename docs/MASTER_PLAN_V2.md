...
1. **Extensión de `useFormAutoSave` como utilidad única** (aprobada: "hook robusto", no extracción de sub-componentes). Firma `useFormAutoSave(formRef, delay=700, watch?: () => string)` → retorna `flush: () => void`. Retrocompatible (callers actuales y el test pasan solo `formRef`/`delay`). Tres capacidades nuevas:
   - **`watch?: () => string`** — para forms cuyo payload sale de estado→JSON (Normas `buildPoliciesJson`, Sistemas `handleSubmit` desde estado): la firma de cambio pasa a `serializeForm(form) + " " + watch()`, así el efecto por-render detecta el cambio de estado que el diff de FormData solo no ve. Evaluado lazy (función), nunca enumera campos.
