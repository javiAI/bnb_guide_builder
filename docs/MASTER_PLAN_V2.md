...
1. **Extensión de `useFormAutoSave` como utilidad única** (aprobada: "hook robusto", no extracción de sub-componentes). Firma `useFormAutoSave(formRef, delay=700, opts?: { watch?: () => string; requireValid?: boolean })` → retorna `flush: () => void`. Retrocompatible (callers actuales y el test pasan solo `formRef`/`delay`). Cuatro capacidades nuevas:
   - **`watch?: () => string`** — para forms cuyo payload sale de estado→JSON (Normas `buildPoliciesJson`, Sistemas `handleSubmit` desde estado): la firma de cambio pasa a `serializeForm(form) + "\0" + watch()`, así el efecto por-render detecta el cambio de estado que el diff de FormData solo no ve. Evaluado lazy (función), nunca enumera campos.
   - **`requireValid?: boolean` (default `true`)** — `false` para Sistemas: los `required` HTML de los renderers de subtipo (5 campos en `system_subtypes.json`, p.ej. WiFi SSID) son hint UI; `updateSystemAction` persiste JSON parcial (`stripNulls`), así que el gate `checkValidity()` no debe bloquear el form entero (un sistema recién creado tiene campos vacíos). Normas no lo necesita (sin `required` HTML).
