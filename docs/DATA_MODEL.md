...
- `LocalEventSourceLink` (Rama 13B — provenance per-source por evento)
  - `id`, `eventId` (FK `LocalEvent.id` con `onDelete: Cascade`), `propertyId` (FK `Property.id` con `onDelete: Cascade` — matchea la convención del resto de tablas property-scoped y previene drift frente a `LocalEvent.propertyId`), `source` (`"predicthq" | "firecrawl" | "ticketmaster" | ...`), `sourceExternalId` (id nativo del proveedor), `sourceUrl`, `confidence` Float, `providerMetadata` JSONB (`nativeCategory`, `nativeTypes[]`, `confidence`, `retrievedAt`), `retrievedAt`, timestamps
  ...
  - **Reconciliación stale property-wide**: después de upsertar todos los events+links del tick, cualquier link preexistente cuya clave `(source, sourceExternalId)` no vuelva a aparecer en ese tick se borra (`deleteMany` por ids diffados contra `tickLinkKeys`). Links cuya clave sí reaparece sobreviven — incluso cuando la canonicalización los reasignó a otro `eventId` (el upsert ya actualizó su `eventId` upstream).
