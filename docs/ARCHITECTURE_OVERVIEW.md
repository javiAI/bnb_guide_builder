...
### PWA + offline (rama 10I — estable desde 2026-04-19)

Capa de instalación + caché que extiende la guía pública sin tocar el modelo de render. La presencia/ausencia del service worker no cambia el modelo de render: HTML y secciones tier 1 son idénticos online y offline. Tier 2 (thumbs) y tier 3 (media md/full) son best-effort — pueden fallar offline si no se han cacheado previamente.

- **Aislamiento per-slug**: el SW se sirve desde un route handler dinámico (`src/app/g/[slug]/sw.js/route.ts`), no desde `public/sw.js` global. Cada propiedad instala un worker con `scope: /g/<slug>/` y header `Service-Worker-Allowed: /g/<slug>/`. Cache names llevan `slug` en el namespace (`guide-<slug>-tier{1,2,3}-<version>`), así dos guías instaladas en el mismo origen no se contaminan.
- **Versioning**: el placeholder `__SW_VERSION__` del template se rellena con `GuideSearchIndex.buildVersion` (rama 10H). Mismo hash determinístico = mismo SW idéntico byte-a-byte; cambio de contenido (label/snippet/keyword/id) flipea el hash y dispara el `update` flow. `Cache-Control: no-cache` en el route handler garantiza el refetch en cada navegación — es el contrato de invalidación real, no un query param (`?v=`) en la URL de registro: añadir query param registraría un SW nuevo en lugar de actualizar el existente. Rollback/unpublish también re-emiten el SW con el hash anterior. Para propiedades sin versión publicada, el fallback de versión es el hash SHA-1 del slug (no `v0` compartido entre guías inéditas).
- **Tres tiers de caché** (declarados en `taxonomies/guide_sections.json` vía `offlineCacheTier: 1|2|3`):
  - **Tier 1 — shell + tier-1 sections** (`gs.essentials`, `gs.arrival`, `gs.checkout`, `gs.emergency`): precache en `install` (HTML root, `/offline`, manifest); HTML responde con stale-while-revalidate; Next static + iconos + manifest cache-first. Es el set garantizado offline.
  - **Tier 2 — media thumbnails**: SWR con cap (`TIER2_MAX_ENTRIES = 12`); las thumbs más antiguas se evictan al insertar la entrada 13.
  - **Tier 3 — media md/full**: network-first con timeout (`TIER3_NETWORK_TIMEOUT_MS = 2000`), fallback al cache si existe, `Response.error()` si no. No es crítico-offline.
...
7. **Tokens y mock-ups son MVP mientras Liora no esté activo.**
   Los tokens actuales en `src/app/globals.css` (legacy `--color-primary-*`/`--color-neutral-*`/etc.) y los mock-ups en `docs/FEATURES/GUEST_GUIDE_UX.md` son referencias operativas — no ground-truth congelado. La fuente de verdad futura es `design-system/foundations/tokens/` (entregado, listo para arrancar 16A). Las ramas funcionales en vuelo (hoy: 13C `feat/guide-maps-embedded`, items futuros de `FUTURE.md`) **no consolidan decisiones visuales finales** y priorizan arquitectura + comportamiento + a11y + reuse de primitivos existentes sobre fidelidad visual.
