...
Capa de instalación + caché que extiende la guía pública sin tocar el modelo de render. La presencia/ausencia del service worker no cambia ningún output: lo que el huésped ve online y offline es idéntico (la red solo refresca; el SW solo replays el último válido).
