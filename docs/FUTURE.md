...
## 29. Equipamiento — 31 amenities configurables invisibles por mismatch scopePolicy↔destination (fix en 16I-6)
...
8. **Derived sin fuente restantes.** `am.hangers` (Perchas — inventario puro: candidata a `amenity_configurable` + scopePolicy real) y los 4 consumibles `am.shampoo`/`am.conditioner`/`am.body_soap`/`am.shower_gel` (supplies: candidatos a `amenity_configurable` property-wide o a `moved_to_guide_content`) siguen `derived_from_space` sin ningún `sf.*` que los respalde. 16I-4 cerró los casos con respaldo real (`sf.hair_dryer`, `sf.safe`, `sf.mosquito_screens`, `sf.outdoor_shower` + guard `SPACE_BACKED_FIELD`).
