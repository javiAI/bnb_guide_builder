...
4. **Quick-fix in-context** (DIFERIDO, rama futura `feat/overview-quick-fix`). Slide-over para resolver blockers atómicos (WiFi, backup de acceso, horarios) sin abandonar Resumen; hoy todo deep-linka a otra pestaña. **Nota de implementación**: `savePropertyAction`/`saveAccessAction` son full-replace → un quick-fix seguro necesita **nuevas server actions parciales** (upsert wifi en `sys.internet`, add-access-backup, set-check-in/out) con auth/audit, no un mini-form contra las actuales (riesgo de pérdida de datos). El de WiFi sí es seguro con `updateSystemAction` (merge de `detailsJson`).
