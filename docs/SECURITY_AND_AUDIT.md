...
Este documento cubre **visibilidad de datos** (qué campo puede salir a qué audiencia) y **audit log** (qué se registra de las mutaciones). Una tercera dimensión — **autenticación y autorización del actor que ejecuta la request** — está cerrada en lo transversal con Fase 15 (15A–15E: operator auth, ownership guards, capability primitive, audit + rate-limit, import apply). Lo que sigue **abierto** es deuda incremental sobre esa base: roles granulares dentro del workspace, MFA, audit reads/queries para el operator, retention policy del AuditLog y audit en cron jobs internos. Ver §0.2 para el detalle. Esta sección es explícita sobre qué hay y qué falta para que ninguna PR se presente como "protegida" sin que lo esté.
